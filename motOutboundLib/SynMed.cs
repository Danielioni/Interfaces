...
namespace motOutboundLib
{
    public static class SynMed
    {
        ...
        */
        ...
        public static async Task Login()
        {
            try
            {
                ...
                using (var scope = container.BeginLifetimeScope())
                {
                    ...
                    var authService = scope.Resolve<IAuthenticationService>();
                    await authService.LoginAsync("mot", "mot");
                }
            }
            catch(Exception ex)
            {
        ...
        public static async Task GetPatientList()
        {
            using (var scope = container.BeginLifetimeScope())
            {
                var query = scope.Resolve<IEntityQuery<User>>();
                var allPatients = await query.QueryAsync(new QueryParameters<Patient>());
            }
        }

        public static async Task GetRxList(Patient p)
        {

            using (var scope = container.BeginLifetimeScope())
            {
                var query = scope.Resolve<IEntityQuery<Rx>>();

                var rxes = await query.QueryAsync(
                    new QueryParameters<Rx>(rx => rx.Status != RxStatus.Discountinue, //filter rxes by status (or any other field if necessary)
                        r => r.RxDosageRegimen.DoseSchedule.DoseScheduleItems, //These expressions defines data which should be included into response
                        r => r.Drug,
                        r => r.Patient.Address,
                ...
                {
                    Console.WriteLine(
                        $"Patient: {rx.Patient.DisplayName}; Drug NDC: {rx.Drug.NdcNumber}; Rx #: {rx.RxSystemId}; Rx type: {rx.RxDosageRegimen.RxType}; quantity: {rx.QuantityWritten};");
                }
            }
        }
}
